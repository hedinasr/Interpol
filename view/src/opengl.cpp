...
void help() {
  ...
  printf("\n");
  printf("Lumiere : \n");
  printf("    alt+fleches ou pageup,pagedown : bouge la mumi�re\n");
  printf("\n");
  printf("Touches : \n");
...
//! A appeller juste avant la boucle principale (Main Loop) et après la création
//! de la fenêtre
...
void parse(const char *file) {
  ...
  if (tokenParLigne <= 0) {
    std::cout << "dimension doit etre > 0 (il y a " << tokenParLigne << ")"
              << std::endl;
    exit(1);
  }
...
int main(int argc, char **argv) {
  ...
  // Creer une fenetre
  window = glutCreateWindow("Interpol");
  ...
  // quand une touche speciale est pressée
